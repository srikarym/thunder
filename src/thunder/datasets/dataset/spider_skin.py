...
def download_spider_skin(root_folder: str):
    import subprocess
    from huggingface_hub import snapshot_download
    snapshot_download(repo_id="histai/SPIDER-skin", repo_type="dataset", local_dir=root_folder, local_dir_use_symlinks=False)

    # unpack the tar files 
    # run cat spider-skin.tar.* | tar -xvf -

    result = subprocess.run(
        f"cat {root_folder}/spider-skin.tar.* | tar -xvf - -C {root_folder}",
        shell=True,
        capture_output=True,
        text=True,
    )
    ...
    import glob
    import os
    tar_files = glob.glob(os.path.join(root_folder, "spider-skin.tar.*"))
    ...
    cmd_to_run = (f"python {root_folder}/scripts/convert_to_imagenet.py "
                  f"--data_dir {root_folder}/SPIDER-skin "
...
def create_splits_spider_skin(base_folder: str, dataset_cfg: dict) -> None:
    """
    Generating data splits for the spider skin dataset.
    ...
    spider_skin_data_splits = init_dict()

    # Retrieving all images and labels
    spider_skin_folder = os.path.join(base_folder, "spider_skin")
    (
        train_val_images,
        train_val_labels,
    ) = get_data_from_folder_recursive(spider_skin_folder, "center_crop/train", dataset_cfg)
    ...
    spider_skin_data_splits["val"]["images"], spider_skin_data_splits["val"]["labels"] = (
        train_val_images[val_mask].tolist(),
        train_val_labels[val_mask].tolist(),
    )
    spider_skin_data_splits["train"]["images"], spider_skin_data_splits["train"]["labels"] = (
        train_val_images[~val_mask].tolist(),
        train_val_labels[~val_mask].tolist(),
    )
    # Test samples
    (
        spider_skin_data_splits["test"]["images"],
        spider_skin_data_splits["test"]["labels"],
    ) = get_data_from_folder_recursive(spider_skin_folder, "center_crop/test", dataset_cfg)

    # dont check dataset characteristics 

    # Few-shot training data
    spider_skin_data_splits = create_few_shot_training_data(spider_skin_data_splits)

    # Saving dict
    save_dict(spider_skin_data_splits, os.path.join(base_folder, "data_splits", "spider_skin.json"))
