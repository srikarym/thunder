...
def download_spider_thorax(root_folder: str):
    import subprocess
    from huggingface_hub import snapshot_download
    snapshot_download(repo_id="histai/SPIDER-thorax", repo_type="dataset", local_dir=root_folder, local_dir_use_symlinks=False)
...
def create_splits_spider_thorax(base_folder: str, dataset_cfg: dict) -> None:
    ...
    (
        train_val_images,
        train_val_labels,
    ) = get_data_from_folder_recursive(spider_thorax_folder, "center_crop/train", dataset_cfg)
    ...
    spider_thorax_data_splits["val"]["images"], spider_thorax_data_splits["val"]["labels"] = (
        train_val_images[val_mask].tolist(),
        train_val_labels[val_mask].tolist(),
    )
    spider_thorax_data_splits["train"]["images"], spider_thorax_data_splits["train"]["labels"] = (
        train_val_images[~val_mask].tolist(),
        train_val_labels[~val_mask].tolist(),
    )
    # Test samples
    ...
    ) = get_data_from_folder_recursive(spider_thorax_folder, "center_crop/test", dataset_cfg)
    ...
    # Saving dict
    save_dict(spider_thorax_data_splits, os.path.join(base_folder, "data_splits", "spider_thorax.json"))
